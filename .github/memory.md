...
| 0040 | Content calendar (month view) | InReview | [plan](../.plan/0040-content-calendar.md) | [#15](https://github.com/elitale/coldsocial/issues/15) | [#59](https://github.com/elitale/coldsocial/pull/59) | Product Developer | `GET /calendar` (`calendar.index`, auth+verified+persona). `CalendarController@index` groups the user's `scheduled_at` posts by their **local** day (user tz), month-scoped via `?month=YYYY-MM` (regex-validated, else current month), UTC bounds in the query. `calendar/index` page = **dependency-free** month grid (Sun–Sat, built from `firstWeekday`+`daysInMonth` — no date lib), per-day chips (time + platform + excerpt) → `posts.show`, prev/today/next nav, today highlighted; **Calendar** sidebar item. Timezone day-placement (2099-08-31 20:00 UTC → Sep 1 for Asia/Kolkata). Stacked on #58→#57→#56→main. CI green (152 tests / 536 assertions) |
