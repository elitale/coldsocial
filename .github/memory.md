...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 0044 | Studio P1: text composer + live preview | InReview | [plan](../.plan/0044-studio-text-preview.md) | [#65](https://github.com/elitale/coldsocial/issues/65) | [#66](https://github.com/elitale/coldsocial/pull/66) | Product Developer | First slice of the **Post Generator Studio** epic (#64). `GET /studio` composer for a **LinkedIn text post** + **live `LinkedInPreview`** (avatar/name, `#hashtag` highlight, see-more, engagement bar, light+dark). `StudioController` create/generate/store: AI caption via `GenerateStudioDraft` (reuses `TextGenerator`+`PersonaVoice`, graceful no-model error), save draft or schedule. `PlatformSpec` registry (LinkedIn charLimit 3000, hashtags 3–5) shared by client counter + server validation. Extracted `ScheduleTime::fromUserInput()` from #14 (2nd consumer: `PostController@schedule` + studio). **Post now** disabled (needs #19). YAGNI cuts: no `format` column (→P3), no preview registry (→P7). **Studio** sidebar item. Off `main`. CI green (189 tests / 696 assertions) |
| 0043 | Platform OAuth credentials (CLI) | InReview | [plan](../.plan/0043-platform-credentials.md) | [#62](https://github.com/elitale/coldsocial/issues/62) | [#63](https://github.com/elitale/coldsocial/pull/63) | Product Developer | DB-stored per-platform OAuth **app** creds via CLI, mirroring the `php artisan ai` console. `platform_credentials` table + `PlatformCredential` model (secret **encrypted**+hidden, unique per platform, `last_tested_at`/`test_passed`/`test_message`). Commands `social:credential:set` (`--test`), `:test {platform?}`, `:list`, `:remove {--force}` + `social` menu (attribute `#[Signature]` + Prompts). `LinkedInOAuth` now resolves creds **DB-first, config fallback** + `testCredentials()` (client-credentials probe: `invalid_client`→fail, else recognised→pass). Discrete commands scriptable (silent redirect default + flags) since `$this->artisan()` reports interactive. larastan quirk: `where(...)->first()` inferred non-null → used ternary not `?->`. No new dep. Stacked on `feature/0042`. CI green (159 tests / 534 assertions). **Follow-up (ICP feedback):** reworked `social` to be **menu-first** with a per-platform status overview (was force-prompting for the secret on first run); added an `enabled` flag (+ `:enable`/`:disable` commands, shared `ResolvesPlatformCredential` trait); the hub now shows **available** only when connectable **and** enabled creds exist (redirect enforces it). CI green (164 tests / 557 assertions) |
| 0042 | Disconnect a platform | InReview | [plan](../.plan/0042-disconnect-platform.md) | [#18](https://github.com/elitale/coldsocial/issues/18) | [#61](https://github.com/elitale/coldsocial/pull/61) | Product Developer | `DELETE /connections/{platform}` → `ConnectionController@destroy`: find the user's connection (404 if none), best-effort `LinkedInOAuth::revoke()` in try/catch, then delete; owner-scoped (cross-user → 404). Connected card gets a **Disconnect** button + shadcn `AlertDialog` (new local component via unified `radix-ui`) → `router.delete` → card returns to **Connect**. Reuses the #17 hub/enum; no new npm package. Stacked on `feature/0041`. CI green (149 tests / 506 assertions) |
| 0041 | Connections hub (OAuth, LinkedIn first) | InReview | [plan](../.plan/0041-connections.md) | [#17](https://github.com/elitale/coldsocial/issues/17) | [#60](https://github.com/elitale/coldsocial/pull/60) | Product Developer | `/connections` page (own sidebar item) + `SocialPlatform` enum registry (Linkedin/Instagram/Tiktok/Youtube/Facebook) with `label()`/`connectable()` — UI renders from `cases()`, **no switch on platform names**. Status per user: `connected`/`available`/`coming_soon`. LinkedIn OAuth via a `LinkedInOAuth` service on the built-in **`Http` client (no Socialite** — latest Socialite caps at guzzle 7, app is on guzzle 8; avoided the downgrade). `PlatformConnection` model (encrypted+hidden access/refresh tokens, unique per user+platform) + migration + factory. Routes: index + `{platform}/redirect` + `{platform}/callback` (enum-bound, non-connectable → 404); session OAuth `state` verified; brand `PlatformIcon` SVGs; inline success/error banner. Others show **Coming soon**. Branched off `feature/0038` (main is red pre-#56). CI green (143 tests / 492 assertions) |
